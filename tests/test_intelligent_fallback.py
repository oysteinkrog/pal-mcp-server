...
class TestIntelligentFallback:
    ...
    def test_prefers_openai_o3_mini_when_available(self):
        """Test that gpt-5.6-terra is preferred when OpenAI API key is available (based on new preference order)"""
        ...
        assert fallback_model == "gpt-5.6-terra"  # Based on new preference order: gpt-5.6-terra before gpt-5.5
    ...
    def test_prefers_gemini_flash_when_openai_unavailable(self):
        """Test that the newest Gemini Flash is used when only Gemini API key is available"""
    ...
    def test_auto_mode_conversation_memory_integration(self):
        ...
        with (
            ...
            with patch("utils.model_context.ModelContext") as mock_context_class:
                ...
                # Verify that ModelContext was called with gpt-5.6-terra (the intelligent fallback based on new preference order)
